...
const WEB_SHUTTLES_JSON = "/var/www/shipyard_fro_usr/data/www/shipyard.frontierstation14.com/storage/app/shuttles/shipyard_data.json";
...
async function init() {
  ...
  ShipyardTypes.forEach((file) => {
    ...
    // Используем полный путь файла как ключ вместо только имени файла
    ShuttlePaths[file.toLowerCase()] = relativePath;
    // Также сохраняем под именем файла для обратной совместимости
  ...
  try {
    ...
    fs.writeFileSync(WEB_SHUTTLES_JSON, JSON.stringify(ShipyardData, null, 4), "utf8");
    console.log(Tags.info + chalk.green(`ShipyardData.json saved to server: ${WEB_SHUTTLES_JSON}`));
  } catch (error) {
    console.log(Tags.error + chalk.red(`Failed to save ShipyardData.json to server: ${error.message}`));
  }
  ...
  const Queue = setInterval(() => {
    ...
    if (AllShuttleToRender.length === 0 && CurrentInstances === 0) {
      ...
      if (EditedShuttles.length !== 0) {
        EditedShuttles.forEach((shuttle) => {
          ...
          const RelativePath = ShuttlePaths[shuttleFile.toLowerCase()] || ShuttlePaths[shuttle.toLowerCase() + ".yml"];
          if (RelativePath) {
    ...
    if (CurrentInstances < MaxInstances) {
      ...
      console.log(chalk.blue(`Starting MapRenderer for ${ShuttleName.split(".")[0]}, Taking ${PrettyPrintNumber(CurrentInstances + 1)} Slot, now at ${CurrentInstances + 1}/${MaxInstances} Instances, ${AllShuttleToRender.length} left to render`));
      // Добавляем префикс Maps/_NF/Shuttles/ к пути файла для MapRenderer
      ...
      const outputDir = fs.existsSync(path.dirname(WEB_RENDERS_DIR)) ? WEB_RENDERS_DIR : path.join(__dirname, "ShuttleRenders");
...
function RenameMappedFile(shuttle) {
  ...
  const baseRenderDir = fs.existsSync(path.dirname(WEB_RENDERS_DIR)) ? WEB_RENDERS_DIR : path.join(__dirname, "ShuttleRenders");
  
  let ShipyardPath = path.join(baseRenderDir, shuttle);
  ...
  if (fs.existsSync(ShuttleFile)) {
    // НЕ переименовываем файл, оставляем как есть без -0
    console.log(Tags.info + chalk.green(`Found rendered file: ${ShuttleFile}`));
  } else {
    // The Linux version seem to uppercase the first letter of the shuttle name
    ShipyardPath = path.join(baseRenderDir, shuttle.replace(/^./, str => str.toUpperCase()));
    ...
    if (fs.existsSync(ShuttleFile)) {
      ...
      if (fs.existsSync(ShipyardPath)) {
        ...
        if (fileToRename) {
          const foundPath = path.join(ShipyardPath, fileToRename);
          console.log(Tags.info + chalk.green(`Found rendered file: ${foundPath}`));
        } else {
